...
-(NSInteger)tableView:(UITableView *)pTableView numberOfRowsInSection:(NSInteger)section {
    ...
    //    }
    return manager.supportMsgList.count;
}
...
-(UITableViewCell *)tableView:(UITableView *)pTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //        return loadMoreCell;
    //    }
    SupportMsgCell *cell = [[SupportMsgCell alloc] init];
    //    if (tableData != nil)
    //    {
    SupportMsgItem *item =  [manager.supportMsgList objectAtExistIndex:indexPath.row];
    [cell setCell:item];
    //    }
    return cell;
}
...
-(void)tableView:(UITableView *)pTableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //    }
    if( self.delegate ){
        [self.delegate supportMsgDelegate:indexPath.row];
    }
    return;
}
