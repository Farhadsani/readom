...
- (void)requestAfterGetTopicDetail{
    ...
    if (cell && [cell isKindOfClass:[CollectCell class]]) {
        ...
        if (opType == topic_like) {
            ...
            //获取话题，获取当前的一条内容
            [self requestNoteDetail:cell.dataItem.noteID];
        }
    }
}
...
- (BuddyStatusNoteItemFrame *)dict2NoteItemFrame:(NSDictionary *)inf{
    ...
    noteItem.topic = [inf objectForKey:@"topic"];
    noteItem.topicid = [[inf objectForKey:@"topicid"] longValue];
    noteItem.noteid = [[inf objectForKey:@"noteid"] longValue];
    noteItem.like_count = [[inf objectForKey:@"like_count"] intValue];
    noteItem.note = [inf objectForKey:@"note"];
    noteItem.ctime = [inf objectForKey:@"ctime"];
    
    NSArray *imgArr = [inf objectForKey:@"imgs"];
    NSMutableArray *imgs = [NSMutableArray array];
    for (NSString *img in imgArr) {
        [imgs addObject:img];
    }
    noteItem.imgs = [imgs copy];
    
    NSArray *thumbArr = [inf objectForKey:@"thumbs"];
    NSMutableArray *thumbs = [NSMutableArray array];
    for (NSString *thumb in thumbArr) {
        [thumbs addObject:thumb];
    }
    noteItem.thumbs = thumbs;
    
    
    NSDictionary *userDic = [inf objectForKey:@"user"];
    ...
    noteItem.user = user;
    NSArray *tagArr = [inf objectForKey:@"tags"];
    ...
    NSArray *placeArr = [inf objectForKey:@"place"];
    ...
    noteItem.place = places;
    
    noteItem.liked = [[inf objectForKey:@"liked"] boolValue];
