...
local CitySelectionViewController = class("CitySelectionViewController", require(QMapGlobal.app.packageRoot .. "/controllers/ViewController"))
...
function CitySelectionViewController:ctor(param)
    ...
    self._viewClassPath = self.packageRoot .. "/citySelection/CitySelectionView"
...
function CitySelectionViewController:_createCities(cityids, cityidOfShowing)
	if cityids and #cityids > 0 then
		...
		for i = 1, #self._cityids do
			...
			local cityData = require(self.packageRoot .. "/data/City").new(QMapGlobal.systemData.mapData[cityid])
...
function CitySelectionViewController:cityBallDownloadTapped(cityID)
  ...
  --       end)
		...
		self.navigationController:push(self.packageRoot .. "/citySelection/OneMsgDialogController")
