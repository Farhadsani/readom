...
mapLayerFrame = mapLayerFrame or {}
...
function MapLayer:ctor(param)
    ...
    local viewPath = self.packageRoot .. "/views/"
    ...
    if not fu:isFileExist(csbMapPathName) then
        csbMapPathName = "ui/" .. csbMap
    end
    ...
    -- 区域
    self._regions = self._mapNode:getChildByName("regions")
    self._regions:setVisible(false)
    self._regions:setLocalZOrder(3)

    self:initRegionsEvent()

    -- self._regionsMask = cc.LayerColor:create(cc.c4b(0, 0, 0, 0))

    -- mark layer
    self._markLayer = require(viewPath .. "MapMarkLayer").new()
    ...
    self.categoryType = CategoryType_sight
    self:addMarkNameLayer()
    self:showCategoryLayer()

    -- popup menu layer
    self._popupMenuLayer = require(viewPath .. "PopupMenuLayer").new()
    ...
    if mapLayerFrame and mapLayerFrame.pos then
        mapLayerFrame.aPoint = mapLayerFrame.aPoint or self._mapNode:getAnchorPoint()
        self._mapNode:setAnchorPoint(mapLayerFrame.aPoint)
        self._mapNode:setPosition(mapLayerFrame.pos)
    end
    if mapLayerFrame and mapLayerFrame.scale then
        self._mapNode:setScale(mapLayerFrame.scale)
    end
...
-- 初始化点击区域的事件
function  MapLayer:initRegionsEvent(  )
    -- body
    local childrens = self._regions:getChildren()
    if childrens then
        print("------------------------------")
        for k, children in pairs(childrens) do
            children:setSwallowTouches(false)
            children:setBackGroundColorType(0)
            children:setBackGroundColor(cc.c3b(255,255,255))
            local isMove = false
            children:addTouchEventListener(function(sender, event)
                -- dump(event)
                -- sender:
                if event == 0 then
                    isMove = false
                elseif event == 1 then
                    isMove = true
                elseif event == 2 then
                    -- local pB = sender:getTouchBeganPosition()
                    -- local pE = sender:getTouchEndPosition()
                    -- print("开始的点。。。。", pB.x, pB.y)
                    -- print("结束的点。。。。", pE.x, pE.y)
                    if not isMove then   --这个地方要不要改，具体看测试情况
                    ...
                    --             math.abs(pTemp.y - beginPointY) < QMapGlobal.TouchAffectRange then
                        ...
                        local data = self.categoryDatas[name]
                        if data and next(data) then
                            self._delegate:onClickRegion(name, function (  )
                                -- body
                            end)
                        end
                    end
...
function MapLayer:addMarkNameLayer( categoryDatas )
    if self._categoryLayer then
        self._categoryLayer:setVisible(false)
        -- print("MapLayer:addMarkNameLayer 设置_categoryLayer 为false")
    end
    if not self._markNameLayer then
        self._markNameLayer = require(self.viewPath .. "MapMarkName").new()
        self._markNameLayer:setDelegate(self._delegate)
        self._markNameLayer:addTo(self._mapRoot)
    end
    -- print("addMarkNameLayer...........")
    self.nameLayerIsShow = false
    self.prohibitNameLayerShow = false

    -- self.categoryDatas = categoryDatas or self.categoryDatas
    ...
    if allSprites then
        for k, sprite in pairs(allSprites) do
            ...
            -- local id = tonumber(sprite:getName())
            local id = sprite:getName()
            local data = self.categoryDatas and self.categoryDatas[id] or {}
            local style = 0
            if next(data) then
                style = 1
            end
            -- print(id, x,y, newPos.x, newPos.y)
            local showName = QMapGlobal.DataManager:getShowNameForSight( self.cityID, id)
            local isSight = QMapGlobal.DataManager:isSight( self.cityID, id) 
            table.insert(datas, {regionID = id , cname = showName, pos = newPos, sprite = sprite, style = style, data = data, isSight = isSight})
        end
    end
...
function MapLayer:showCategoryLayer( categoryDatas )
    dump(categoryDatas)
    if self._markNameLayer then
        self._markNameLayer:setVisible(false)
    end
    if not self._categoryLayer then
        self._categoryLayer = require(self.viewPath .. "MapMarkName").new()
        self._categoryLayer:addTo(self._mapRoot)
        self._categoryLayer:setDelegate(self._delegate)
        self._categoryLayer:setVisible(false)
    end
    -- print("showCategoryLayer...........")
-- self._mapNode:setVisible(false)
    self.categoryDatas = categoryDatas or self.categoryDatas
    local datas = {}
    local allChildrens = self._regions:getChildren()
    if allChildrens then
        for k, children in pairs(allChildrens) do
            local x,y = children:getPosition()
            local newPos = self._categoryLayer:convertToNodeSpace(self._regions:convertToWorldSpace(cc.p(x,y)))

            local name = children:getName()
            local data = self.categoryDatas and self.categoryDatas[name] or {}
            if data and next(data) then
                table.insert(datas, {regionID = name ,cname = name, pos = newPos, sprite = children, isSight = false, data = data, style = 1})
            else
                -- table.insert(datas, {cname = name, pos = newPos, sprite = children, isSight = true, data = data, style = 0})
            end
        end
    end

    self._categoryLayer:setMapSpaceToMarkSpaceConvertFunc(function(pointInMapSpace)
        return self._categoryLayer:convertToNodeSpace(self._regions:convertToWorldSpace(pointInMapSpace))
    end)
    -- local sp = display.newSprite("res/starTest1.png")
    -- sp:addTo(self._categoryLayer)
    -- sp:setPosition(cc.p(0  ,  0))
    self._categoryLayer:showName(datas)
    self._categoryLayer:setVisible(true)
    ...
    if self._allSenicSpotsFlash ~= flashed then
        ...
        if flashed then
            for k, node in pairs(children) do
                -- print("&&&&&&&&&&&&&&&&&&")
                -- local id = tonumber(node:getName())
                if QMapGlobal.DataManager:isSight( self.cityID, node:getName()) then
...
function MapLayer:_addSenicSpotCloneAboveLocationsMask(sprite)
    ...
    local duplicatedSprite = display.newSprite(frame)
...
function MapLayer:_setMapScale(newScale)
    ...
    if newScale ~= currentScale then
        ...
        -- print(newScale , s, maxScale, minScale, (maxScale + minScale)/4)
        if self.prohibitNameLayerShow or self.categoryType ~= CategoryType_sight then
            self._markNameLayer:setNameVisible(false)
            -- print("self._markNameLayer 被设置为false", self.categoryType)
            self.nameLayerIsShow = false
        else
            self._markNameLayer:setNameVisible(newScale > s)
            -- print("self._markNameLayer 被设置为", newScale > s)
            self.nameLayerIsShow = (newScale > s)
        end

        if self._categoryLayer then
            if self.categoryType == CategoryType_sight then
                -- print("self._categoryLayer 被设置为false", self.categoryType)
                self._categoryLayer:setVisible(false)
            else
                -- print("self._categoryLayer 被设置为", newScale > s)
                self._categoryLayer:setVisible(newScale > s)
            end
        end
...
function MapLayer:_guardSBLPositionAssign(newSBLPosition)
    ...
    -- print("设置mapNode的位置 in _guardSBLPositionAssign", newSBAPosition.x, newSBAPosition.y)
    self._mapNode:setPosition(newSBAPosition)
    mapLayerFrame.pos = newSBAPosition

    if self._mapScaleUpdated then
        self._markLayer:refresh()
        self._markNameLayer:refresh()
        self._categoryLayer:refresh()
        ...
        self._categoryLayer:deltaMove(delta)
    end
...
function MapLayer:_setMapAnchorToPoint(pointInMap)
    ...
    -- print("设置mapNode的位置 in _setMapAnchorToPoint", newAnchorBasedPosition.x, newAnchorBasedPosition.y)
    mapLayerFrame.aPoint = newAnchorPoint
    mapLayerFrame.pos = newAnchorBasedPosition
end
...
function MapLayer:_singleTap(x, y)
    ...
    if (self._itemTapped) then
        if self.categoryType == CategoryType_sight then
            ...
            print("当前点击了", self._itemTapped:getName())
...
function MapLayer:introduceWillShow( show )
    ...
    if show or self.categoryType ~= CategoryType_sight then
        -- print("introduceWillShow 中设置。。。_markNameLayer， false")
        self._markNameLayer:setNameVisible(false) 
    else
        -- print("introduceWillShow 中设置。。。_markNameLayer", self.nameLayerIsShow)
        self._markNameLayer:setNameVisible(self.nameLayerIsShow) 
    end
end

function MapLayer:refreshData(categoryType, datas)
    -- self:addMarkNameLayer(datas)
    -- print("function MapLayer:refreshData(categoryType, datas)..............", categoryType)
    self.categoryType = categoryType
    if categoryType == CategoryType_sight then
        self._regions:setVisible(false)
        -- self:setGestureRecognitionEnabled(true)
        self:setAllSenicSpotsFlash(true)
        self:addMarkNameLayer()
    else
        self._regions:setVisible(true)
        -- self:setGestureRecognitionEnabled(false)
        self:setAllSenicSpotsFlash(false)
        self:showCategoryLayer(datas)
    end
end
