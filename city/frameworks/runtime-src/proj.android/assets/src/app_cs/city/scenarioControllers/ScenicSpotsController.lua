...
function ScenicSpotsController:reLoadView( param )
	-- body
...
function ScenicSpotsController:tappedScenicSpotWithName(scenicSpotName)
	...
	if self._scenicSpotNameOfCurrentPopup ~= scenicSpotName then
		...
		-- end
		-- dump(sightData.desc)
		if sightData and next(sightData) then
			local descJson = json.encode(sightData.desc)
			-- print(json.encode(sightData.desc))

			print(sightData.sightid, sightData.cname)
			self._parentController:onClickSight(sightData.sightid, sightData.cname, descJson, function (  )
				print("景点介绍返回。。。。。。")
				self:tappedElseWhere()
			end)
		else
		...
		-- openSightIntro(sightData.sightid, sightData.cname, descJson, function (  )
		-- 	print("景点介绍返回。。。。。。")
		-- 	self:tappedElseWhere()
		-- end)
...
function ScenicSpotsController:onClickMapMarkName( regionType, regionID )
	-- body
	print("function ScenicSpotsController:onClickMapMarkName(  )")
	if regionType == SpaceType_sight then

		local sightData = self._parentController.city:getSightData(regionID)
		local descJson = json.encode(sightData.desc)
		self._parentController:onClickSight(regionID, sightData.cname, descJson, function (  )
			-- print("景点介绍返回。。。。。。")
			self:tappedElseWhere()
		end)
	elseif regionType == SpaceType_area then
		self:onClickRegion(regionID, function( ... )
			-- body
		end)
	end
end

function ScenicSpotsController:onClickRegion(name, callBack)
	self._parentController:onClickSight(name, nil, nil, function (  )
		print("区域返回。。。。。。")
		self:tappedElseWhere()
		if callBack then
			callBack()
