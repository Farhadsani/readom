...
function LoadView:ctor(param)
    ...
    -- print_r(self.loadingbar)    --ccui.LoadingBar

    self.sp1:setPosition(cc.p(display.width/2, display.height/2 +220 -100))
...
function LoadView:setPercent( percent )
	-- body
	if percent > 100 then percent = 10 end
	if percent < 0 then percent = 0 end
	self.loadingbar:setPercent(percent)
	self.txtPercent:setString(tostring(percent) .. "%")
end
