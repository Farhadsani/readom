...
local netWorkManager = require("app/data/NetWorkManager").new()   -- 操作网络
local dataBaseManager = require("app/data/DataBaseManager").new()  -- 操作数据库
local tableFields = require("app/resData/TableFields")   -- 加载字段表
...
function DataManager:loadData(  )
    ...
    if not dataBaseManager:isTableExist("city") then  -- 不存在，则创建  ，导入内置数据
        ...
        for _, city in pairs(CityTable) do
            ...
            local sightPath = downloadPath .. "map/" .. city.name .. "/" .. city.name .. "sight.json"
            print("读取json文件。。", sightPath)
            if fu:isFileExist(sightPath) then
                print("存在json文件，需要从json中获取数据！")
                sights = self:getSightFromJsonFile(sightPath)
            else   -- 没有对应的sight文件
...
function DataManager:getSightFromJsonFile( jsonFile )
    ...
    -- local fullPath = fileUtil:fullPathForFilename(sightPath)
...
function DataManager:initSystemData()
