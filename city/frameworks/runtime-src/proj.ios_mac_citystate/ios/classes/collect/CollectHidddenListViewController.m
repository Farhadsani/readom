...
- (void)beginSelectedCollectCell:(CollectCell *)CollectCell{
    ...
    [self showConfirmSelectedView:arr action:@selector(clickSelectedItem:) hasBackColor:NO clickBackDismiss:NO animation:YES];
}
    ...
    if ([[req.data_dict objectOutForKey:k_r_url] hasSuffix:k_api_feed_user_list]) {
        ...
        for (CollectCell * cell in _collectCellList) {
            for (NSDictionary *dict in [self.data_dict objectOutForKey:@"res"]) {
                if ([dict[@"feedid"] intValue] == cell.dataItem.feedid) {
...
- (void)requestList{
    ...
    NSDictionary * d = @{k_r_url:k_api_feed_user_list,
                         k_r_delegate:self,
                         k_r_postData:dict,
                         };
    [[ReqEngine shared] tryRequest:d];
