    ...
    if ([inf objectOutForKey:V_Frame]) {
        ...
        if (v_width) {
            ...
            if ([UIView isViewClass:v_right_view]) {
                width = width - (toView(v_right_view).superview.width - toView(v_right_view).x);
//                width = width - (toView(v_right_view).width);
            }
        ...
        if (v_height) {
            ...
            if ([UIView isViewClass:v_next_view]) {
                height = height - (toView(v_next_view).superview.height - toView(v_next_view).y);
//                height = height - (toView(v_next_view).height);
            }
...
+ (NSDictionary *)combineDict:(NSDictionary *)dic{
    if ([NSDictionary isNotEmpty:dic] && [dic objectOutForKey:V_Common_Style] && [NSDictionary isNotEmpty:[dic objectOutForKey:V_Common_Style]]) {
        NSMutableDictionary * md = [[[NSMutableDictionary alloc] initWithDictionary:dic] autorelease];
        [md removeObjectForKey:V_Common_Style];
        [md setValuesForKeysWithDictionary:[dic objectOutForKey:V_Common_Style]];
        return md;
    }
    return dic;
}
...
+ (UILabel *)label:(NSDictionary *)inf{
    ...
    lab.textColor = [UIColor color:darkblack_color];
...
+ (UIButton *)button:(NSDictionary *)inf{
    ...
    [button setTitleColor:[UIColor color:darkblack_color] forState:UIControlStateNormal];
    if ([inf objectOutForKey:V_Color]) {
...
//    }
    ...
    textField.textColor = [UIColor color:white_color];
    if ([inf objectOutForKey:V_Color]) {
        textField.textColor = [UIColor color:[inf objectOutForKey:V_Color]];
    }
    
    if ([inf objectOutForKey:V_PlaceHoldTextColor]) {
        [textField setValue:[UIColor color:[inf objectOutForKey:V_PlaceHoldTextColor]] forKeyPath:@"_placeholderLabel.textColor"];
    }
    
    if ([inf objectOutForKey:V_PlaceHoldTextFont] && [[inf objectOutForKey:V_PlaceHoldTextFont] isKindOfClass:[UIFont class]]) {
        [textField setValue:[inf objectOutForKey:V_PlaceHoldTextFont] forKeyPath:@"_placeholderLabel.font"];
    }
    
    CGFloat font_size = k_defaultFontSize;
    if ([inf objectOutForKey:V_Font_Size]) {
        font_size = [[inf objectOutForKey:V_Font_Size] floatValue];
    }
    if ([inf objectOutForKey:V_Font_Weight] && [[inf objectOutForKey:V_Font_Weight] integerValue] == FontWeightBold) {
...
//            }
    ...
    if ([inf objectOutForKey:V_LeftImage]) {
        ...
        UILabel * im = [self label:@{V_Text:[inf objectOutForKey:V_LeftText],
                                     V_Font_Size:([inf objectOutForKey:V_Font_Size])?[inf objectOutForKey:V_Font_Size]:num(k_defaultFontSize),
