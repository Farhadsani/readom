    ...
    if ([NSDictionary isNotEmpty:obj]) {
        for (NSString * key in [obj allKeys]) {
            if ([obj objectOutForKey:key]) {
                [self setNonEmptyObject:[obj objectForKey:key] forKey:key];
            }
        }
    }
}
...
- (id)objectOutForKey:(NSString *)anAttribute{
    ...
    if (value) {
        if ([value isKindOfClass:[NSNull class]]) {
            ...
            NSString * st = [NSString stringWithFormat:@"%@", value];
//            if ([st floatValue] == [value floatValue]) {
//                return st;
//            }
//            else{
//                
...
//            if ([NSString stringWithFormat:@"%@", value]) {
