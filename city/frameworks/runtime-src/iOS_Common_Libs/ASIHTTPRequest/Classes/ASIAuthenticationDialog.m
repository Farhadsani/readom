...
- (UIViewController *)presentingController
{
	if (!presentingController) {
		...
		UIWindow *window = [[[UIApplication sharedApplication] windows] objectAtExistIndex:0];
...
- (UITextField *)textFieldInRow:(NSUInteger)row section:(NSUInteger)section
{
	return [[[[[self tableView] cellForRowAtIndexPath:
			  ...
			  contentView] subviews] objectAtExistIndex:0];
}
...
- (void)showTitle
{
	UINavigationBar *navigationBar = [[[self view] subviews] objectAtExistIndex:0];
	UINavigationItem *navItem = [[navigationBar items] objectAtExistIndex:0];
...
- (void)show
{
	...
	[[[[[self tableView] cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]].contentView subviews] objectAtExistIndex:0] becomeFirstResponder];
...
- (void)presentNextDialog
{
	if ([requestsNeedingAuthentication count]) {
		ASIHTTPRequest *nextRequest = [requestsNeedingAuthentication objectAtExistIndex:0];
