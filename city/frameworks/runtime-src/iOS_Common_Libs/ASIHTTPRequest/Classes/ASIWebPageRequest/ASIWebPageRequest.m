...
- (void)requestFinished
{
	...
	contentType = [[contentType componentsSeparatedByString:@";"] objectAtExistIndex:0];
...
- (NSString *)relativePathTo:(NSString *)destinationPath fromPath:(NSString *)sourcePath
{
	...
	for (i=0; i<[sourcePathComponents count]; i++) {
		sourcePathComponent = [sourcePathComponents objectAtExistIndex:i];
		if ([destinationPathComponents count] > i) {
			destinationPathComponent = [destinationPathComponents objectAtExistIndex:i];
			if (![sourcePathComponent isEqualToString:destinationPathComponent]) {
				...
				for (i2=i+1; i2<[destinationPathComponents count]; i2++) {
					newPath = [newPath stringByAppendingPathComponent:[destinationPathComponents objectAtExistIndex:i2]];
				}
				break;
			}
		}
...
- (NSString *)cachePathForRequest:(ASIWebPageRequest *)theRequest
{
	...
	// This is a fallback for when we don't have a download cache - we store the external resource in a file in the temporary directory
		...
		return [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtExistIndex:0] stringByAppendingPathComponent:[md5 stringByAppendingPathExtension:@"html"]];
	}
}
